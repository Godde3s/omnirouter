// Entry point of the Qwen bridge (package gbridge).
//
...
func Run() {
	...
	flag.BoolVar(&config.AgentMode, "agent-mode", config.AgentMode, "Enable agent mode: translate tools & roles for agent compatibility (modern shim by default)")
	...
	accountsLine := "guest mode (no cookies needed)"
	if accounts != nil {
		accountsLine = fmt.Sprintf("%d account(s), round-robin + failover", accounts.Len())
	}
	fmt.Printf(`
╔═══════════════════════════════════════════════════════════════╗
║           Gemini Free API Bridge Server Started                 ║
╠═══════════════════════════════════════════════════════════════╣
║  Upstream:      gemini.google.com (3.6 Flash / 3.5 Flash-Lite / 3.1 Pro)           ║
║  Accounts:      %-46s║
