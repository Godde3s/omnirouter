...
<title>Gemini-Free-API — داشبورد</title>
<style>
...
<header>
  <div class="logo"><div class="dot">✨</div>
    <h1>Gemini-Free-API<small>پل رایگان به Gemini 3.6 Flash، 3.5 Flash-Lite و 3.1 Pro — بدون مرورگر، فقط HTTP</small></h1>
  </div>
...
<footer>
  ...
  · مستندات کامل: <a href="https://github.com/Godde3s/gemini-free-api" target="_blank">GitHub</a><br>
...
function renderAccounts(s){
  ...
  if(!pool.enabled||!s.accounts||!s.accounts.length){
    box.innerHTML='<div class="acct"><div class="av">👻</div><div class="who"><b>حالت مهمان</b>'+
      '<span>بدون کوکی — حالت مهمان زنده تست شده است؛ برای سهمیه کامل کوکی‌های __Secure-1PSID را در GEMINI_COOKIES بگذار</span></div>'+
...
async function loadModels(){
  try{
    ...
    const prefer=["gemini-3.6-flash","gemini-3.1-pro","gemini-3.5-flash-lite"];
    const first=prefer.find(p=>ids.includes(p))||ids[0]||"gemini-3.6-flash";
    const list=ids.length?ids:[first];
    $("model").innerHTML=list.map(id=>"<option>"+esc(id)+"</option>").join("");
    $("model").value=first;
  }catch(e){ $("model").innerHTML="<option>gemini-3.6-flash</option>"; }
}
loadModels();
