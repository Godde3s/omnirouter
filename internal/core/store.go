...
type APIKey struct {
        ...
        // Client identifies the OAuth device-flow connection that minted this
        // key ("cline", "kilo", "hermes", …). Empty = created by hand.
        Client string `json:"client,omitempty"`
        // UserCode is the device-flow code the user approved for this key.
...
// asks for a code, the user approves it in the dashboard/consent page, and
// polling returns a freshly minted virtual API key.
type DeviceGrant struct {
        DeviceCode string `json:"device_code"`
        UserCode   string `json:"user_code"`
        Client     string `json:"client"` // client id (cline, kilo, hermes, …)
...
const deviceGrantTTL = 15 * 60 // seconds — codes expire like GitHub's (15 min)
...
func NewStore(dir string) *Store {
        ...
        if s.data.DeviceGrants == nil {
                s.data.DeviceGrants = []DeviceGrant{}
        }
        return s
}
...
// pruneExpiredGrants drops stale pending grants (called on every touch).
func (s *Store) pruneExpiredGrants() {
        now := time.Now().Unix()
        keep := s.data.DeviceGrants[:0]
        for _, g := range s.data.DeviceGrants {
                if g.Status == "pending" && now > g.ExpiresAt {
                        continue // expired pending grants vanish
                }
                keep = append(keep, g)
        }
        s.data.DeviceGrants = keep
}

// CreateDeviceGrant registers a new pending grant for a client.
func (s *Store) CreateDeviceGrant(deviceCode, userCode, client string) DeviceGrant {
        s.mu.Lock()
        defer s.mu.Unlock()
        ...
        g := DeviceGrant{
                ...
                ExpiresAt:  time.Now().Unix() + deviceGrantTTL,
        }
        s.data.DeviceGrants = append(s.data.DeviceGrants, g)
        s.dirty = true
        _ = s.saveLocked()
        return g
}

// LookupDeviceGrant fetches a grant by its long device code.
func (s *Store) LookupDeviceGrant(deviceCode string) (DeviceGrant, bool) {
        s.mu.Lock()
        defer s.mu.Unlock()
        s.pruneExpiredGrants()
        for _, g := range s.data.DeviceGrants {
                if g.DeviceCode == deviceCode {
                        return g, true
                }
        }
...
// LookupUserGrant fetches the most recent grant for a user code.
func (s *Store) LookupUserGrant(userCode string) (DeviceGrant, bool) {
        s.mu.Lock()
        defer s.mu.Unlock()
        s.pruneExpiredGrants()
        var best *DeviceGrant
        for i := range s.data.DeviceGrants {
                if s.data.DeviceGrants[i].UserCode == userCode {
                        if best == nil || s.data.DeviceGrants[i].CreatedAt > best.CreatedAt {
                                best = &s.data.DeviceGrants[i]
                        }
                }
        }
        if best == nil {
...
// ResolveDeviceGrant approves/denies a pending grant and, on approve, mints
// the linked virtual key (named after the client + user code). Returns the
// updated grant.
func (s *Store) ResolveDeviceGrant(userCode, decision string) (DeviceGrant, bool) {
        s.mu.Lock()
        defer s.mu.Unlock()
        s.pruneExpiredGrants()
        var best *DeviceGrant
        for i := range s.data.DeviceGrants {
                g := &s.data.DeviceGrants[i]
                if g.UserCode == userCode && g.Status == "pending" {
                        if best == nil || g.CreatedAt > best.CreatedAt {
                                best = g
                        }
                }
        }
        ...
        // approve → mint a dedicated key for this connection
        key := APIKey{
                Key:       "sk-omni-" + randomHex(24),
                Name:      best.Client + " · " + best.UserCode,
                CreatedAt: time.Now().Unix(),
                Enabled:   true,
                Client:    best.Client,
                UserCode:  best.UserCode,
        }
...
// DeleteDeviceGrant removes a consumed grant record (after successful poll).
func (s *Store) DeleteDeviceGrant(deviceCode string) {
        s.mu.Lock()
        defer s.mu.Unlock()
        for i := range s.data.DeviceGrants {
                if s.data.DeviceGrants[i].DeviceCode == deviceCode {
                        s.data.DeviceGrants = append(s.data.DeviceGrants[:i], s.data.DeviceGrants[i+1:]...)
                        s.dirty = true
                        _ = s.saveLocked()
                        return
                }
...
// PendingGrants returns live pending grants (for the dashboard panel).
func (s *Store) PendingGrants() []DeviceGrant {
        s.mu.Lock()
        defer s.mu.Unlock()
        s.pruneExpiredGrants()
        var out []DeviceGrant
...
// ConnectionKeys returns the keys minted through OAuth connections.
func (s *Store) ConnectionKeys() []APIKey {
        s.mu.RLock()
        defer s.mu.RUnlock()
        var out []APIKey
