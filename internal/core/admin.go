...
func registerAdminRoutes(mux *http.ServeMux, st *Store, reg *Registry, fw *forwarder, cfg *Config) {
        ...
        mux.HandleFunc("/admin/api/freebuff", adminAPI(func(w http.ResponseWriter, r *http.Request) {
                switch r.Method {
                case "GET":
                        writeJSON(w, 200, fbridge.Status())
                case "POST":
                        var body struct {
                        ...
                        // FREEBUFF_TOKENS env stays authoritative when set.
                        if len(fbridge.TokensFromEnv()) > 0 {
                                writeJSON(w, 409, map[string]interface{}{"error": "FREEBUFF_TOKENS env is set — it overrides dashboard tokens"})
                                return
                        }
                        clean := make([]string, 0, len(body.Tokens))
                        for _, t := range body.Tokens {
                                if t = strings.TrimSpace(t); t != "" {
                                        clean = append(clean, t)
                                }
                        }
                        st.SetFreebuffTokens(clean)
                        fbridge.SetTokens(clean)
                        writeJSON(w, 200, map[string]interface{}{"ok": true, "tokens": len(clean)})
                case "DELETE":
                        if len(fbridge.TokensFromEnv()) > 0 {
                                writeJSON(w, 409, map[string]interface{}{"error": "FREEBUFF_TOKENS env is set — unset it to disconnect"})
                                return
                        }
                        st.SetFreebuffTokens([]string{})
                        fbridge.SetTokens(nil)
                        writeJSON(w, 200, map[string]interface{}{"ok": true})
                default:
                        http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
                }
